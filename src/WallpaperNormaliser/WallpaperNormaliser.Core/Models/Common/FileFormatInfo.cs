...
public sealed record FileFormatInfo(EFileFormat Format)
{
    ...
    public static IReadOnlySet<string> SupportedExtensions { get; } =
        new HashSet<string>(_extensionMap.Keys, StringComparer.OrdinalIgnoreCase);

    /// <summary>Recognised MIME types, lower-case. Case-insensitive on lookup.</summary>
    public static IReadOnlySet<string> SupportedMimeTypes { get; } =
        new HashSet<string>(_mimeTypeMap.Keys, StringComparer.OrdinalIgnoreCase);

    public static FileFormatInfo? FromExtension(string extension)
        => _extensionMap.TryGetValue(extension, out EFileFormat format)
            ? new FileFormatInfo(format)
            : null;

    public static FileFormatInfo? FromMimeType(string mimeType)
        => _mimeTypeMap.TryGetValue(mimeType, out EFileFormat format)
            ? new FileFormatInfo(format)
            : null;
}
